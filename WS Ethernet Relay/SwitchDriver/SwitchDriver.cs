...
//
// ASCOM Switch driver for WaveshareModbus POE ETH Relay
//
...
namespace ASCOM.Waveshare_Modbus_POE_ETH_Relay.Switch
{
    ...
    //
    // Your driver's DeviceID is ASCOM.WaveshareModbus_POE_ETH_Relay.Switch
    //
    // The COM Guid attribute sets the CLSID for ASCOM.WaveshareModbus_POE_ETH_Relay.Switch
    // The COM ClassInterface/None attribute prevents an empty interface called _WaveshareModbus_POE_ETH_Relay from being created and used as the [default] interface
    //
    ...
    public class Switch : ReferenceCountedObjectBase, ISwitchV2, IDisposable
{
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="WaveshareModbus POE ETH Relay"/> class. Must be public to successfully register for COM.
    /// </summary>
    public Switch()
    {
        try
        {
            ...
            tl = new TraceLogger("", "WaveshareModbus POE ETH Relay.Driver"); // Remove the leading ASCOM. from the ProgId because this will be added back by TraceLogger.
            SetTraceState();
        ...
        catch (Exception ex)
        {
            ...
            MessageBox.Show($"{ex.Message}", "Exception creating ASCOM.WaveshareModbus POE ETH Relay.Switch", MessageBoxButtons.OK, MessageBoxIcon.Error);
        }
    }
